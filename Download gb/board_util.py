...
class GoBoardUtil(object):
    ...
    def generate_moves(board,color):
        ...
        return move
    @staticmethod
    def surrlib(board,neighbors,color):
        #color = board.current_player
        #print("thisruns")
        myneigh = []
        for move in neighbors:
            if board.get_color(move) == 0:
                myneigh.append(move)
        return myneigh        
    
    @staticmethod
    def my_neighbours(board, neighbors,color):
        #color = board.current_player
        #print("thisruns")
        myneigh = []
        for move in neighbors:
            if board.get_color(move) == color:
                myneigh.append(move)
        return myneigh
    ...
    @staticmethod
    def return_lib(board,move,color):
        lib = []
        #color = board.current_player
        vistedneigh = []
        notvisneigh = []
        notvisneigh.append(move)
        while len(notvisneigh) > 0:
            current = notvisneigh.pop()
            currentneigh = board._neighbors(current)
            for cn in currentneigh:
                #print(cn)
                if board.board[cn] == color:
                    if cn not in vistedneigh:
                        notvisneigh.append(cn)
                elif board.board[cn] == 0:
                    lib.append(cn)
                else:
                    pass
                #if cn == color, check if in visited, if not visited, append to notvis
                #elif cn == lib, append to lib
                #else pass?
            vistedneigh.append(current)
        return lib
    
    @staticmethod
    def group_neighbor(board, move, capneigh, prev_move,color, group=1):
        #color = board.current_player
        opponent_color = GoBoardUtil.opponent(color)
        #print(opponent_color)
        if prev_move != None:
            neigh = board._neighbors(move)
        else:
            neigh = []
        ...
        for mov in neigh:
            capneigh.append(mov)
            if group != 1:
                if board.board[mov] == color:
                    if mov not in capneigh and mov not in prev_move:
                        #print(mov)
                        GoBoardUtil.group_neighbor(board, mov, capneigh, prev_move,color, group)
            if board.board[mov] == opponent_color:
                if mov in prev_move:
                    continue
                else:
                    GoBoardUtil.group_neighbor(board, mov, capneigh, prev_move,color, group)
    ...
    @staticmethod
    def generate_all_policy_moves(board,pattern,check_selfatari,color = board.current_player):
        """
        ...
        """
        capturemoves = GoBoardUtil.generate_moves(board,color)
        ...
        if prev_move != None:
            ...
            capneigh = GoBoardUtil.group_neighbor(board, prev_move, capneigh, movelist,color)
        else:
            neighbors = []
        
        capture_move, liberty = GoBoardUtil.liberty_moves(board, capneigh)
        #print(liberty)
        #print(capture_move)
        ...
        # BUY THAT ONEeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
        #getting list of friendly neighbours
        myneigh = GoBoardUtil.my_neighbours(board,neighbors,color)
        #color = board.current_player
        #print(color)
        #print(myneigh)
        adef = []
        for movb in myneigh:
            liblist = GoBoardUtil.return_lib(board,movb,color)
            if len(liblist) == 1:
                #print(GoBoardUtil.selfatari(board, liblist[0],1))
                if not GoBoardUtil.selfatari(board,liblist[0],color):
                    #print(board.check_legal(liblist[0],color))
                    if board.check_legal(liblist[0],color):
                        adef = adef +liblist
        
        surlib = GoBoardUtil.surrlib(board,neighbors,color)
        surlibn = []
        opp = []
        for lib in surlib:
            surlibn += board._neighbors(lib)
        set(surlibn)
        list(surlibn)
        #print(GoBoardUtil.opponent(color))
        for lib in surlibn:
            #print(lib)
            if board.board[lib] == GoBoardUtil.opponent(color):
                opp.append(lib)
        #print(opp)
        for movb in opp:
            liblist = GoBoardUtil.return_lib(board,movb,GoBoardUtil.opponent(color))
            if len(liblist) == 1:
                if not GoBoardUtil.selfatari(board,liblist[0],color):
                    #print(board.check_legal(liblist[0],color))
                    if board.check_legal(liblist[0],color):
                        adef = adef +liblist                
        if len(adef) != 0:
            return adef, "AtariDefense"

        #capture_moves, liberty = GoBoardUtil.liberty_moves(board, myneigh)
        #print (capture_moves)
        #for  in capture_move:
            #GoBoardUtil.checkforatari()
            #check for liberties
            #if liberties = 1, and playing that move gives more liberties, return atari defence
        ################################################################################
        ################################################################################
        ################################################################################
